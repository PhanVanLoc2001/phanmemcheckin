...
@section('content')
    <div class="container-p-x flex-grow-1 container-p-y">
        <div class="button_create mb-2">
            @can('user-create')
            <a href="{{route('users.create')}}"><button type="submit" class="btn btn-primary me-2">Thêm</button></a>
            @endcan
        </div>
        <div class="card ">
                            ...
                            <td>
                                @if(!empty($user->getRoleNames()))
                                    @foreach($user->getRoleNames() as $v)
                                        <label class="text-dark"><span class="badge bg-success">{{ $v }}</span></label>
                                    @endforeach
                                @endif
                            </td>
                            ...
                            <td>
                                <div class="d-inline-block text-nowrap">
                                        ...
                                        <form class="inline-block" style="display: inline-block;width: 100%;"
                                            ...
                                            onsubmit="confirmDeleteUser(event)">
                                            @csrf
                                            @method('DELETE')
